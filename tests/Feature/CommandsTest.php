...
class CommandsTest extends TestCase
{
    ...
    {
        $commands = [
            'vendor-cleanup:config' => 'Report which published config files differ from their vendor originals',
            'vendor-cleanup:migration' => 'Report which published migration files differ from their vendor originals',
            'vendor-cleanup:lang' => 'Report which published lang files differ from their vendor originals',
            'vendor-cleanup:view' => 'Report which published view files differ from their vendor originals',
        ];

        foreach ($commands as $commandName => $expectedDescription) {
            $output = $this->artisan('list', ['--format' => 'json'])
                ->run();

            $this->artisan('list')
                ->expectsOutputToContain($commandName);
        }
    }
